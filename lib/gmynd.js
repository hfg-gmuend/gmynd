...
function createNode(nodeName, attr, draw) {
  attr = attr || {};
  if (draw === undefined) draw = true; // let draw default to true
...
function createInstance(sourceElem, attr, draw) {
  attr = attr || {};
  if (draw === undefined) draw = true; // let draw default to true
  var elem = createNode('use', {}, draw);
...
function appendTransform(elem, transforms) {
  ...
  var newTransform = existingTransform.concat(transforms);
  setTransform(elem, transforms);
  return newTransform;
}
